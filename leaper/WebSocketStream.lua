...
--print(string.format("0x%x 0x%x", headerbuff[0], headerbuff[1]))
	...
	if frameHeader.PayloadLen < 126 then
		...
		self.SourceStream:WriteBytes(headerbuff, 2);
	...
	-- otherwise it is absent.
	if frameHeader.MASK then
		local byteswritten, err = self.SourceStream:WriteBytes(frameHeader.maskingkey, 4);
...
WebSocketStream_t.WriteFrame = function(self, message, FIN, opcode, MASK)
	FIN = FIN or 1;
	opcode = opcode or 1;
	MASK = MASK or 0;
...
--print("WebSocketStream_t.ReadFrame")
	...
	local byteswritten, err = self.SourceStream:WriteBytes(payloaddata, payloadlen);
